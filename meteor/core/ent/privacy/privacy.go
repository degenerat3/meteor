...
// The BotQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type BotQueryRuleFunc func(context.Context, *ent.BotQuery) error

// EvalQuery return f(ctx, q).
func (f BotQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.BotQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.BotQuery", q)
}

// The BotMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type BotMutationRuleFunc func(context.Context, *ent.BotMutation) error

// EvalMutation calls f(ctx, m).
func (f BotMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.BotMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.BotMutation", m)
}
